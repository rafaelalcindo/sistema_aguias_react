...
export function HoraPontoTable({
    title,
    list,
    getHoraPonto,
    setOrderString
    ...
    async function removeHoraPonto(id: number) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire(
                    'Deletado',
                    'A hora cadastrada foi removida com sucesso!.',
                    'success'
                )
                getHoraPonto();
            }
                ...
                <div className="container">
                    <div className="flex flex-row justify-between text-2xl py-4 px-4 ">
                        ...
                        <div className={`buttonAdd`}>
                            <Link to={"/horaponto/add"} className={`text-blue-900`} > Adicionar </Link>
                        </div>
            ...
            <table className="w-full  bg-lightWhite rounded-b-lg  sm:shadow-lg">
                <thead>
                    <tr className={`flex flex-col flex-no wrap lg:table-row rounded-l-lg lg:rounded-none mb-5 sm:mb-0 lg:mb-0 border-b-4 border-blue-950`}>
                        ...
                        <th className={`p-3 text-left h-1/6 lg:w-10 text-xs sm:text-base `} >
                            <a
                                onClick={() => organizeDirection('data_programacao')}
                                className={`flex flex-row ${styles.titleCol}`}
                            >
                                Data &nbsp;
                                {arrowDirection('data_programacao')}
                        ...
                        <th className={`p-3 text-left h-1/6 lg:w-10 text-xs sm:text-base `} >
                            <a
                                onClick={() => organizeDirection('hora_programacao')}
                                className={`flex flex-row ${styles.titleCol}`}
                            >
                                Hora &nbsp;
                                {arrowDirection('hora_programacao')}
                ...
                <tbody>
                    {
                        ...
                        list.map((item, index) => (
                            <tr key={index} className={`flex flex-col flex-no wrap lg:table-row mb-5 sm:mb-0 line ${styles.lineRow}`} >
                                ...
                                <td className="p-3 h-1/6 text-xs sm:text-base">
                                    {transformDateWithoutTime(item.data_programacao)}
                                </td>
                                <td className="p-3 h-1/6 text-xs sm:text-base">
                                    {item.hora_programacao}
                                </td>
                                <td className="p-3 h-1/6 text-xs sm:text-base">
                                    {item.pontos}
                                </td>
